...
export function handleDbError(err: any): never {
    ...
    if (err?.code === "23505") {
        ...
        const isEmail =
            detail.toLowerCase().includes("(email)") || detail.toLowerCase().includes("email");
    ...
    if (err?.code === "23503") {
        throw new ConflictException({
            ...
            messageEn: "Cannot complete this action because the record is referenced by other data.",
