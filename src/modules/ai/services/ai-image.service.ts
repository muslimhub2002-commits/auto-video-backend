...
export class AiImageService {
  ...
  async generateImageForSentence(dto: GenerateImageDto, userId: string) {
    ...
    const notProtectedCharacters = referencedCanonicalCharacters.filter(
      (character) =>
        !character.isSahaba && !character.isProphet && !character.isWoman,
    );

    const noHumanFiguresRule =
      'Do NOT show any character, person, or humanoid figure. ' +
      'Symbolize the sentence through environmental storytelling, meaningful objects';
    console.log(protectedCharacters.length, 'protectedCharacters', hasMultipleReferencedCharacters, 'hasMultipleReferencedCharacters');
    const showNoHumanFiguresRule = (protectedCharacters.length === 1 && !hasMultipleReferencedCharacters) || (hasMultipleReferencedCharacters && protectedCharacters.length > 1)
    const protectedCharactersRole = showNoHumanFiguresRule ? noHumanFiguresRule
      : referencedCharacterKeys.length &&
        protectedCharacters.length &&
        referencedCharacterKeys.length !== protectedCharacters.length
