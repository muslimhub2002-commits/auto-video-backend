...
describe('alignByWordCount', () => {
  ...
  it('ignores bracketed cues, underscores, and dashes when aligning sentence words', async () => {
    mockedIsReplicateWhisperXEnabled.mockReturnValue(true);
    mockedIsAssemblyAiEnabled.mockReturnValue(false);
    mockedAlignWithReplicateWhisperX.mockResolvedValue([
      { text: 'Hello', startSeconds: 0, endSeconds: 0.4 },
      { text: 'world', startSeconds: 0.4, endSeconds: 0.9 },
      { text: 'again', startSeconds: 0.9, endSeconds: 1.3 },
    ]);

    const timings = await alignAudioToSentences({
      openai: null,
      audioPath: 'unused.mp3',
      sentences: [{ text: '[whisper] Hello—_world_ -again' }],
      audioDurationSeconds: 1.5,
      withTimeout: async (promise) => promise,
      disableRenderer: true,
    });

    expect(timings[0].text).toBe('[whisper] Hello—_world_ -again');
    expect(timings[0].words?.map((word) => word.text)).toEqual([
      'Hello',
      'world',
      'again',
    ...
    expect(timings[0].endSeconds).toBeCloseTo(1.3, 5);
    expect(timings[0].words?.[2].endSeconds).toBeCloseTo(1.3, 5);
  });

  it('ignores bracketed cues, underscores, and dashes in synthetic fallback timings', () => {
    const [timing] = alignByWordCount(
      [{ text: '[pause] Hello—_world_ -again' }],
      2,
    );

    expect(timing.text).toBe('[pause] Hello—_world_ -again');
    expect(timing.words?.map((word) => word.text)).toEqual([
      'Hello',
      'world',
      'again',
    ]);
